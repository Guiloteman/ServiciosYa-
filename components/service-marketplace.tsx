...
export function ServiceMarketplace() {
  ...
  async function requestService() {
    if (!clientName.trim() || !clientAddress.trim()) {
      setServiceError('El nombre y la dirección son obligatorios.')
      return
    }

    if (paymentMethod !== 'cash' && paymentMethod !== 'mercadopago') {
      setServiceError('Selecciona una forma de pago.')
      return
    }
    ...
    const payload = {
      ...
      clientPhone: clientPhone.trim() || 'indefinido',
      clientEmail: clientEmail.trim() || 'indefinido',
  ...
  <div className="mx-auto flex h-[72px] max-w-[1320px] items-center justify-between px-5 lg:px-8">
    ...
    <button type="button" aria-label="Abrir menú" aria-expanded={showMobileMenu} onClick={() => setShowMobileMenu((open) => !open)} className="rounded-xl border border-slate-200 p-2 text-[#163d75] md:hidden">
      {showMobileMenu ? <X size={20} /> : <Menu size={20} />}
    </button>
  </div>
  {showMobileMenu && <nav className="border-t border-slate-100 bg-white px-5 py-3 md:hidden"><div className="flex flex-col gap-2 text-sm font-semibold text-slate-600"><a href="#inicio" onClick={() => setShowMobileMenu(false)} className="rounded-lg px-3 py-2 hover:bg-slate-50">Inicio</a><a href="#servicios" onClick={() => setShowMobileMenu(false)} className="rounded-lg px-3 py-2 hover:bg-slate-50">Servicios</a></div></nav>}
